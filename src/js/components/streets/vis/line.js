import { dom, getImagePath, createImageElement, layers, addMapLayer, rankBySentimentScore } from '../../component';
...
import { default as streetlayers } from '../map/layers';

export var currentMin = 0;
export var currentMax = 0;

export var currentStreets = undefined;
export var currentChart = undefined;
export var current_active_point = [];
export var sentimentOptions = { extras: {} };

export default function line(streets) {
    
    currentStreets = streets;
    console.log(sentimentOptions);
    streetlayers.sentimentPoint.clearLayers();
...
function getData (streets) {
    ...
    streets.forEach( street => {
        ...
        for (let i = 0, len = street.sentiments.length; i < len; ++i) {
            // get street data
            let date = street.dates[i];
            // Modify sentiment 
            let sentiment = new Sentiment();
            let score = sentiment.analyze(street.narratives[i], sentimentOptions).score;
            //let score = street.sentiments[i];
            let keyword = street.keywords[i];
            let pos = dates.indexOf(date);
            data[pos] += score;
            radius[pos] += 1;
            keywords[pos].push(keyword);
            narratives[pos].push(street.narratives[i]);
            points[pos].push(street.points[i]);

            // Create images left and right
            let pathLeft = getImagePath(street.videoLNames[i], street.videoLTimes[i]);
            let pathRight = getImagePath(street.videoRNames[i], street.videoRTimes[i]);
            images[pos].push({
    ...
    datasets.forEach( d => {
        ...
        for (let i = 0; i < d.rad.length; ++i) {
            d.rad[i] = normalize(d.rad[i], [min, max], [2, 20]);
        }
...
function getOptions (chart) {
    ...
    // Get current min and current max for chart data
    // Use for Y axis
    ...
    return {
        ...
        title: {
            display: true,
            text: 'Time Varying Sentiment Scores'
        }
    };
}
...
function addClickEvent (canvas, chart) {
    canvas.on('click', (e) => {
        // Remove all sentiment sentences
        ...
        if (activePoint[0]) {
            ...
            if (!chartData.datasets[datasetIndex].select[idx]) {
                ...
                current_active_point.push(activePoint);
            } else {
                ...
                var pos = current_active_point.map( (x) => {
                    return x[0]._index;
                }).indexOf(idx);

                current_active_point.splice(pos, 1);
            }
        }
    });
}
...
// TODO: Need to add dictionary here 
...
function getDictionary (word, score) {
    ...
    updateButton.on('click', function () {
        ...
        // Redraw chart 
        let datasets = getData(currentStreets);
        currentChart.data = datasets;
        currentChart.options = getOptions(currentChart);
        currentChart.update();
        update_table();
    });
        ...
        pos.on('click', function () {
            ...
            if (pos.hasClass('select')) {
                ...
                data.positive[i].narratives.forEach( sentence => {
                    ...
                    streetlayers.sentimentPoint.addLayer(marker);
                    addMapLayer(getCurrentMap(), streetlayers.sentimentPoint);
                ...
                var bounds = streetlayers.sentimentPoint.getBounds();
        ...
        neg.on('click', function () {
            ...
            if (neg.hasClass('select')) {
                ...
                data.negative[i].narratives.forEach( sentence => {
                    ...
                    sentence = sentence.replace("those 2 have warrants", "...");
                    sentence = sentence.replace("we are actually", "...");
                    sentence = sentence.replace("they have to have um they got to fit like the elements of it like they got to be stealing copper piping", "...");
                    ...
                    let sentenceContainer = $('<div/>', {
                        ...
                        'font-size': '12px',
                        'padding': '2px'
                    }).html(sentence.replace("those 2 have warrants", "..."));
...
function getStreetOptions (keywords) {
    ...
    return; //div.append(imageOption, keywordOption);
}

// Create image gallery
function getImageGallery (images, points, narratives) {

    let container = $('<div/>', {
        class: 'sentiment-gallery'
    }).css({
    ...
    if (images.length > 0) {
        ...
        for (let i = 0; i < images.length; ++i) {
            let left = createImageElement(images[i].left, 'sentiment-img', '', '');
            let right = createImageElement(images[i].right, 'sentiment-img', '', '');

            left.addEventListener('mouseover', () => {
                layers.sentimentImage.clearLayers();
                $('.sentiment-img').css({ border: '3px solid #ffffff'});
                left.style.border = '3px solid #ef3b2c';
                // Add pulse icon to the marker
                let marker = L.circleMarker(points[i]).bindTooltip(narratives[i], {
                    className: 'currentTripTooltip',
                }).addTo(getCurrentMap());
                layers.sentimentImage.addLayer(marker);
                addMapLayer(getCurrentMap(), layers.sentimentImage);
                marker.openTooltip();
            });
            ...
            right.addEventListener('mouseover', () => {
                layers.sentimentImage.clearLayers();
                $('.sentiment-img').css({ border: '3px solid #ffffff'});
                right.style.border = '3px solid #ef3b2c';
                // Add pulse icon to the marker
                let marker = L.circleMarker(points[i]).bindTooltip(narratives[i], {
                    className: 'currentTripTooltip',
                }).addTo(getCurrentMap());
...
export function update_table()
{
    ...
    for (let i = 0; i < current_active_point.length; ++i) {
        let active_point = current_active_point[i];
        if (active_point[0]) {
            let idx = active_point[0]._index;
            let datasetIndex = active_point[0]._datasetIndex;
            let chartData = active_point[0]._chart.data;
